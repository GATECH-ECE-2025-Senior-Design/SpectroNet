...
entity i2c_oneshot_ctrl is
	GENERIC(
		iwl : std_logic_vector(1 downto 0) := std_logic_vector(to_unsigned(2#00#, 2)); -- input word length in bits (2'b00=16bit, 2'b01=20bit, 2'b10=24bit, 2'b11=32bit)
		sample_control : std_logic_vector(5 downto 0) := std_logic_vector(to_unsigned(2#001101#, 6)) -- sampling control bits (default 8kHz USB mode; refer to WM8731 manual)
	);
	port(
		resetn          : in  std_logic;
		clk             : in  std_logic;
...
architecture main of i2c_oneshot_ctrl is
	...
	signal interface_format : std_logic_vector(15 downto 0);
	signal sample_cntrl_format : std_logic_vector(15 downto 0);
	
begin

	interface_format <= 
		std_logic_vector(to_unsigned(2#0001111_0#, 8)) 	-- Digital Audio Interface Register Addr
		& std_logic_vector(to_unsigned(2#0101#, 4)) 		-- dsp master, data on 2nd BCLK
		& iwl															-- parameterized input word length
		& std_logic_vector(to_unsigned(2#11#, 2));		-- dsp mode
		
	sample_cntrl_format <=
		std_logic_vector(to_unsigned(2#0001000_000#, 10)) -- Sampling Control Register Addr and pad
		& sample_control;
	...
	commands(1) <= 
		interface_format;  -- dsp master, X-bit (default 16-bit), data on 2nd clock
	commands(2) <= 
		sample_cntrl_format;  -- X mode (default USB), Xkhz sampling (default 8kHz)
	commands(3) <= 
