...
create_clock -name "AUD_BCLK" -period "50.000 MHz" [get_ports AUD_BCLK]
...
# WM8731 Audio Codec (only if we want to use WM8731 in slave mode)
# create_clock -name AUD_XCK -period "____ MHz" [get_ports AUD_XCK]
# create_clock -name AUD_BCLK -period "____ MHz" [get_ports AUD_BCLK]

# Unused, feel free to change name & frequency
